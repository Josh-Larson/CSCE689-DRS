...
#include <resources/DistributedAAR.h>
...
int main(int argc, char *argv[]) {
	...
	auto clientPort = ((argc >= 4) ? uint16_t(std::stoi(argv[3])) : DEFAULT_PORT);
	auto daar = DistributedAAR(serverPort, argc >= 3 ? argv[2] : "", clientPort);
	if (argc >= 3) {
